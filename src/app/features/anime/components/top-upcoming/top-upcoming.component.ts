    ...
    <div class="container mx-auto px-4 py-8">
      ...
      >
        ...
        <app-anime-card
          ...
          [customTopRightContent]="getFavoritesHTML(anime)"
        ></app-anime-card>
        }
      </div>
...
export class TopUpcomingComponent
  ...
  implements OnInit
{
  ...
  constructor() {
    super();
    this.title = "Top Upcoming Anime";
    this.subtitle = "Most Anticipated Upcoming Anime";
  }
  ...
  searchAnime() {
    ...
    this.jikanService.anime
      ...
      .subscribe({
          ...
          const sortedAnimes = result.data
            .filter(
              (anime: {
                ...
                // Filter out anime with no images or extremely low popularity
                anime.images?.jpg?.image_url && (anime.favorites ?? 0) > 100
            )
            .sort(
              (
                a: { favorites: any; popularity: any },
                b: { favorites: any; popularity: any }
              ) => {
                // Sort by favorites first, then by popularity
            ...
            .map((anime: { score: any }) => ({
              ...anime,
              isHovered: false,
              score: typeof anime.score === "number" ? anime.score : 0, // Ensure score is a number
            }));
        ...
        error: (err) => {
          this.error.set(err.message || "Failed to fetch top upcoming anime");
          this.loading.set(false);
        },
      });
  }
