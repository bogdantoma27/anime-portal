...
export class TopPopularComponent
  ...
  implements OnInit
{
  ...
  constructor() {
    ...
    this.subtitle = "Popular Series You Will Love to Watch";
  }
  ...
  searchAnime() {
    ...
    this.jikanService.anime
      ...
      .subscribe({
          ...
          const sortedAnimes = result.data
            .filter(
              (anime: {
                ...
                popularity: any;
              }) => anime.images?.jpg?.image_url && (anime.popularity ?? 0) > 0
            )
            .sort((a: { popularity: any }, b: { popularity: any }) => {
              // Sort by popularity, where lower value means more popular
              return (a.popularity || Infinity) - (b.popularity || Infinity);
            })
            .map((anime: any) => ({
              ...anime,
              isHovered: false,
        ...
        error: (err) => {
          this.error.set(err.message || "Failed to fetch most popular anime");
          this.loading.set(false);
        },
      });
  }
